...
public class DiningPhilosophersProblem implements Runnable {
    ...
    public static void main(String[] args) {
        boolean useWaiter = ((args != null) && (args.length == 1) && ("waiter".equals(args[0]) || "distinguished".equals(args[0])));
        Waiter waiter = (useWaiter ? "waiter".equals(args[0]) ? Waiter.Dumb : Waiter.Distinguished : null);
        List<Philosopher> philosophers = new ArrayList<Philosopher>(5);
        List<Chopstick> chopsticks = new ArrayList<Chopstick>(5);
        Object supplemental = (useWaiter ? (waiter == Waiter.Dumb ? PhilosopherFactory.constructWaiter()
                                                                  : PhilosopherFactory.constructDistinguishedWaiter()) : null);
        for (int i = 0; i < 5; i++) {
            if (useWaiter) {
                if (waiter == Waiter.Dumb) {
                    philosophers.add(PhilosopherFactory.constructWithDumbWaiter((DumbWaiter) supplemental, getName(i)));
                } else {
                    philosophers.add(PhilosopherFactory.constructWithDistinguishedWaiter((DistinguishedWaiter) supplemental,
                            getName(i)));
                }
            } else {
